...
pub async fn get_connector(uri: &str) -> Box<dyn Connector> {
    if uri.contains("mongodb") {
        Box::new(MongodbConnectorBuilder::new(uri).build().await.unwrap())
    } else if uri.contains("postgresql") {
        Box::new(PostgresqlConnectorBuilder::new(uri).build().await.unwrap())
    } else {
        panic!("Other connectors are not implemented");
    }
}
