...
use crate::{
    connectors::base::{
        get_connector, Connector, ConnectorInfo, DatabaseData, DatabaseFetchResult, DatabaseKind,
        DatabaseValue, Object, PaginationInfo,
    },
    log_error,
    managers::{
                ...
                ConnectionEvent::Connect(value) => {
                    ...
                    self.event_sender
                        .send(Event::OnAsyncEvent(tokio::spawn(async move {
                            let connector_new = get_connector(&cloned_value).await;
                            let info = connector_new.get_info();
                            cloned_sender
                            ...
                            cloned_sender
                                .send(Event::OnResourceEvent(ResourceEvent::Update(Box::new(
                                    ConnectorResource {
                                        connector: Some(Arc::new(Mutex::new(connector_new))),
